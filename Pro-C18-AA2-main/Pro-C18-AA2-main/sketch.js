...
function setup() {
  ...
  trex = createSprite(50, windowHeight / 1.7, 20, 50);

  trex.addAnimation("running", trex_running);
  trex.addAnimation("collided", trex_collided);
  trex.scale = 0.5;

  ground = createSprite(windowWidth, windowHeight / 1.5, 400, 20);
  ground.addImage("ground", groundImage);
  ground.x = ground.width / 2;
  ground.velocityX = -(6 + 3 * score / 100);

  gameOver = createSprite(width / 2, height/2, width, 125);
  ...
  invisibleGround = createSprite(windowWidth, windowHeight / 1.45, width * 2, 10);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(width, windowHeight/2, 40, 10);
...
function spawnObstacles() {
  if (frameCount % 60 === 0) {
    var obstacle = createSprite(width, windowHeight/1.55, 10, 40);
    //obstacle.debug = true;
